...
namespace Flexodeal
{
  ...
  namespace Parameters
  {
    ...
    void MuscleProperties::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Materials");
      {
        ...
        prm.declare_entry("Fat constant 1", "323.91",
                          Patterns::Double(),
                          "Fat constant 1 in Neo-Hookean SEF");

        prm.declare_entry("Fat constant 2", "5163.1",
                          Patterns::Double(),
                          "Fat constant 2 in Neo-Hookean SEF");
        
        prm.declare_entry("Fat constant 3", "-3872.9",
                          Patterns::Double(),
                          "Fat constant 3 in Neo-Hookean SEF");
    ...
    void MuscleProperties::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Materials");
      {
        ...
        fat_c2     = prm.get_double("Fat constant 2");
        fat_c3     = prm.get_double("Fat constant 3");
  ...
  protected:
    ...
    {
      return 2 * s_base_fat * (3 * c3_fat * std::pow(trace_b_bar - 3,2)
           + 2 * c2_fat * (trace_b_bar - 3) + c1_fat) * b_bar;
    }
    ...
    {
      return 4 * s_base_fat * (6 * c3_fat * (trace_b_bar - 3) + 2 * c2_fat)
             * outer_product(b_bar,b_bar);
    }
